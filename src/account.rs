...
fn execute_registration_exchange(
    ...
    let client_registration_start_result =
        crypto::opaque::register_start(&mut client_rng, client_password);
...
fn execute_login_exchange(
    ...
    let (credential_finalization_bytes, client_session_key) =
        crypto::opaque::login_finish(client_login_start_result, &credential_response_bytes)
            .map_err(|err| format!("account login error: {:?}", err))?;
